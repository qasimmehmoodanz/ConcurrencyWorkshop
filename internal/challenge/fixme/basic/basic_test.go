...
func TestNilChannel(t *testing.T) {
	...
	ch := make(chan int) // not initialising properly
...
func TestClosedChannelWithoutOkCheck(t *testing.T) {
	...
	for {
		select { // Not checking if channel is closed
		case val, ok := <-ch:
			if !ok {
...
func TestContextWithCancel(t *testing.T) {
	...
	defer cancelFunc() // Ensure the cancel function is called to release resources
...
func TestContextWithTimeout(t *testing.T) {
	...
	case <-time.After(time.Second * 3): // Ensure this timeout is longer than the context timeout
